...
const ProfilePage = ({ user, referralCount, setUser, errorDetails }: { user: UserProfile, referralCount: number, setUser: React.Dispatch<React.SetStateAction<UserProfile | null>>, errorDetails: string | null }) => {
  ...
  return (
    <div className="px-6 space-y-6 pt-6 pb-24">
      ...
      <div className="glass-card p-6 border-white/10 bg-white/5 relative overflow-hidden group">
        ...
        <div className="relative">
          <h3 className="text-[10px] font-black text-[#94a3b8] uppercase tracking-[0.2em] mb-3 flex items-center gap-2">
             <Send className="w-3.5 h-3.5 text-blue-400" />
             Referral Link
          </h3>
          <div className="bg-[#0f172a] p-4 rounded-xl border border-white/5 font-mono text-[10px] text-neutral-400 break-all select-all shadow-inner mb-4">
            {shareLink}
          </div>
        </div>
        
        <div className="flex gap-2 relative h-40">
           <a 
            href={telegramShare}
            target="_blank"
            rel="noopener noreferrer"
            className="flex-1 bg-gradient-to-b from-blue-500 to-indigo-600 text-white rounded-2xl font-black text-[11px] flex flex-col items-center justify-center gap-2 shadow-xl shadow-blue-500/20 active:scale-95 transition-all p-4 text-center"
          >
            <Send className="w-6 h-6" />
            <span className="uppercase tracking-widest leading-tight">SHARE ON TELEGRAM</span>
          </a>
          ...
          <button 
            ...
            className="w-14 bg-[#1e293b] text-white rounded-2xl font-black text-[10px] border border-white/10 active:scale-95 transition-all flex flex-col items-center justify-center gap-2"
          >
            <div className="vertical-text uppercase tracking-widest opacity-60">COPY</div>
          </button>

          <div className="flex-1 bg-white/5 border border-white/5 rounded-2xl p-4 flex flex-col justify-between">
            <div className="flex items-center justify-between">
               <span className="text-[8px] uppercase font-black text-[#94a3b8] tracking-widest">System Stats</span>
               <div className="w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse shadow-[0_0_8px_#22c55e]" />
            </div>
            
            <div className="space-y-3">
              <div>
                <p className="text-[7px] uppercase font-bold text-neutral-500 tracking-tighter">Build Version</p>
                <p className="text-[9px] font-mono text-blue-400/90 font-bold">2024.04.21.BETA</p>
              </div>
              <div>
                <div className="flex items-center justify-between mb-0.5">
                  <p className="text-[7px] uppercase font-bold text-neutral-500 tracking-tighter">Last Server Sync</p>
                  <button 
                    onClick={async () => {
                      if (isSyncing) return;
                      setIsSyncing(true);
                    ...
                    className={cn("p-1 rounded bg-white/5 hover:bg-white/10 transition-colors", isSyncing && "animate-spin")}
                  >
                    <RefreshCcw className="w-2.5 h-2.5 text-neutral-400" />
                  </button>
                </div>
                <p className={cn("text-[9px] font-mono font-bold", isSyncing ? "text-yellow-500" : "text-green-400")}>
                  {(() => {
...
const ArenaMatchmaking = ({ user, onStartBattle }: { user: UserProfile, onStartBattle: (op: any) => void }) => {
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {opponents.map((op, i) => (
            ...
            >
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 rounded-full border-2 border-white/10 overflow-hidden bg-neutral-900">
                   {op.photo_url ? <img src={op.photo_url} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center font-bold">{op.username?.[0]?.toUpperCase()}</div>}
                </div>
                <div>
                   <p className="text-xs font-black text-white">@{op.username}</p>
                   <p className="text-[10px] text-yellow-500 font-bold uppercase">LVL {op.hero_level} {op.hero_class}</p>
                </div>
              </div>
              <button 
                onClick={() => onStartBattle(op)}
                className="px-4 py-2 bg-red-600 text-white text-[10px] font-black uppercase rounded-lg shadow-lg shadow-red-600/20 active:scale-95 transition-all"
              >
                ATTACK
              </button>
            </motion.div>
...
const BattleArena = ({ user, opponent, onFinish, setUser }: { user: UserProfile, opponent: any, onFinish: () => void, setUser: any }) => {
  ...
  if (loading) return (
    <div className="fixed inset-0 bg-[#0f172a] z-[100] flex flex-col items-center justify-center p-10 text-center">
       <div className="w-24 h-24 border-t-4 border-red-600 rounded-full animate-spin mb-6" />
       <h2 className="text-2xl font-black uppercase tracking-tighter text-red-500">Entering Combat Zone</h2>
       <p className="text-xs text-neutral-500 mt-2">Calculating environmental variables and skill matrices...</p>
    </div>
  );
  ...
  return (
    <div className="fixed inset-0 bg-[#0f172a] z-[100] p-6 pt-12 overflow-y-auto">
      {/* Fighters Header */}
      <div className="flex items-center justify-between mb-12">
        <div className="text-center space-y-2 flex-1">
          <div className="w-20 h-20 rounded-2xl bg-blue-500/10 border-2 border-blue-500/30 mx-auto overflow-hidden relative">
             {user.photo_url ? <img src={user.photo_url} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center font-black text-2xl">{user.username?.[0]?.toUpperCase()}</div>}
             <div className="absolute top-0 left-0 bg-blue-500 text-[8px] px-1 font-black">YOU</div>
          </div>
          <p className="text-[10px] font-black uppercase tracking-tight text-white truncate">@{user.username}</p>
        ...
        <div className="px-4 text-2xl font-black text-red-500 italic">VS</div>

        <div className="text-center space-y-2 flex-1">
          <div className="w-20 h-20 rounded-2xl bg-red-500/10 border-2 border-red-500/30 mx-auto overflow-hidden relative">
             {opponent.photo_url ? <img src={opponent.photo_url} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center font-black text-2xl">{opponent.username?.[0]?.toUpperCase()}</div>}
             <div className="absolute top-0 right-0 bg-red-500 text-[8px] px-1 font-black">OPP</div>
          </div>
          <p className="text-[10px] font-black uppercase tracking-tight text-white truncate">@{opponent.username}</p>
          <div className="w-full bg-white/5 h-2 rounded-full overflow-hidden border border-white/5">
            <motion.div 
               animate={{ width: `${(currentRound?.defender_hp || opponent.hero_health) / opponent.hero_health * 100}%` }}
        ...
        <AnimatePresence mode="popLayout">
          {battleData.rounds.slice(0, roundIndex + 1).map((r: any, i: number) => (
            <motion.div 
              ...
              className="glass-card p-4 bg-white/5 border-white/5 flex items-center gap-4"
            >
              <div className="w-8 h-8 rounded-full bg-red-500/10 flex items-center justify-center font-black text-[10px] text-red-500 shrink-0">R{i+1}</div>
              <div>
                <p className="text-[11px] text-white/90 leading-tight">{r.event_msg}</p>
                <div className="flex gap-4 mt-1">
                  <span className="text-[8px] font-black text-blue-400">-{r.defender_damage} DMG</span>
                  <span className="text-[8px] font-black text-red-400">-{r.attacker_damage} TAKEN</span>
                </div>
              </div>
            </motion.div>
          )).reverse()}
      ...
      {isFinished && (
        ...
        >
          <div className="w-full max-w-sm glass-card p-8 text-center border-white/10 bg-gradient-to-tr from-[#1e293b] to-[#0f172a]">
             <div className="p-4 bg-white/5 rounded-2xl mb-6 inline-block">
                {isWin ? <Trophy className="w-16 h-16 text-yellow-500" /> : <Shield className="w-16 h-16 text-neutral-500" />}
             </div>
             <h2 className={cn("text-4xl font-black uppercase mb-2", isWin ? "text-yellow-500" : "text-neutral-500")}>
               {isWin ? "Victory!" : "Defeat"}
             </h2>
             <p className="text-xs text-neutral-400 mb-8 uppercase tracking-widest font-bold">Arena Session Concluded</p>
             
             <div className="grid grid-cols-2 gap-4 mb-8">
               <div className="p-4 bg-white/5 rounded-xl border border-white/5">
                 <p className="text-[10px] text-neutral-500 uppercase font-black mb-1">Rewards</p>
                 <p className="text-xl font-black text-white">+{battleData.reward_gldp.toLocaleString()}</p>
                 <p className="text-[8px] text-yellow-500 font-bold uppercase">GLDp</p>
               </div>
               <div className="p-4 bg-white/5 rounded-xl border border-white/5">
                 <p className="text-[10px] text-neutral-500 uppercase font-black mb-1">Ranking</p>
                 <p className="text-xl font-black text-white">{isWin ? `+${battleData.reward_points}` : `+3`}</p>
                 <p className="text-[8px] text-indigo-400 font-bold uppercase">Activity pts</p>
               </div>
             </div>

             <button 
               onClick={onFinish}
               className="w-full py-4 bg-white text-black rounded-2xl font-black text-xs uppercase tracking-[0.3em] active:scale-95 transition-all shadow-xl shadow-white/10"
             >
               Return to HQ
             </button>
          </div>
...
const HeroTab = ({ user, setUser }: { user: UserProfile, setUser: any }) => {
  ...
  const classes = [
    { 
      ...
      icon: Shield, 
      color: 'blue',
      desc: 'Balanced offense and defense with high survivability.',
      stats: { atk: 'Medium', def: 'Medium', hp: 'High' },
      skills: ['Recovery R3/R6', 'Anti-Archer Defense']
    },
    { 
      name: 'Archer', 
      icon: Target, 
      color: 'orange',
      desc: 'High velocity attacker that exploits Mage weaknesses.',
      stats: { atk: 'High', def: 'Medium', hp: 'Medium' },
      skills: ['Dodge Chance', 'Mage Destroyer']
    },
    { 
      name: 'Mage', 
      icon: Zap, 
      color: 'purple',
      desc: 'Tactical fighter with high defense and elemental burn.',
      stats: { atk: 'Medium', def: 'High', hp: 'Medium' },
      skills: ['25% Reflect/Dodge', 'Warrior Burn Curse']
    }
  ];
  ...
  const handleUpgrade = async () => {
    ...
    if (user.balance < cost) return alert("Insufficient GLDp!");
  ...
  if (!user.hero_class) {
    return (
      <div className="px-6 py-10 space-y-8 animate-in fade-in duration-700">
        <div className="text-center">
          <h2 className="text-3xl font-black gold-gradient uppercase tracking-tighter">Initialize Hero</h2>
          <p className="text-xs text-neutral-500 uppercase font-black tracking-widest mt-1">Select your Combat Template</p>
        </div>
            ...
            >
              <div className="flex items-center gap-5">
                <div className={cn(
                  ...
                  c.color === 'blue' ? "bg-blue-500/10 border-blue-500/30 text-blue-400 shadow-[0_0_20px_rgba(59,130,246,0.3)]" :
                  c.color === 'orange' ? "bg-orange-500/10 border-orange-500/30 text-orange-400 shadow-[0_0_20px_rgba(249,115,22,0.3)]" :
                ...
                <div className="flex-1">
                  ...
                  <div className="flex gap-2 mt-1">
                    {Object.entries(c.stats).map(([k, v]) => (
                      <span key={k} className="text-[7px] uppercase font-black px-1.5 py-0.5 bg-white/5 rounded border border-white/5">
                        {k}: <span className="text-white">{v}</span>
                      </span>
                    ))}
                  </div>
                  <p className="text-[10px] text-neutral-500 mt-2 leading-tight">{c.desc}</p>
                </div>
                <ChevronRight className="w-5 h-5 opacity-20" />
              </div>
            </motion.div>
  ...
  return (
    <div className="px-6 py-6 pb-24 space-y-6">
      ...
      {/* Hero Overview */}
      <div className="glass-card p-6 bg-gradient-to-b from-[#1e293b]/80 to-[#0f172a]/80 border-white/10 relative overflow-hidden group">
        <div className="absolute top-0 right-0 w-32 h-32 bg-yellow-500/10 blur-[60px] pointer-events-none rounded-full -mr-16 -mt-16"></div>
        <div className="flex items-center gap-6">
          <div className="w-24 h-24 rounded-3xl bg-white/5 border border-white/10 flex items-center justify-center p-3 relative">
             {user.hero_class === 'Warrior' && <Shield className="w-full h-full text-blue-400 drop-shadow-[0_0_15px_rgba(59,130,246,0.5)]" />}
             {user.hero_class === 'Archer' && <Target className="w-full h-full text-orange-400 drop-shadow-[0_0_15px_rgba(249,115,22,0.5)]" />}
             {user.hero_class === 'Mage' && <Zap className="w-full h-full text-purple-400 drop-shadow-[0_0_15px_rgba(168,85,247,0.5)]" />}
             <div className="absolute -bottom-2 -right-2 bg-yellow-500 text-black px-2 py-0.5 rounded font-black text-[10px]">LVL {user.hero_level}</div>
          </div>
          <div className="flex-1">
             <h3 className="text-2xl font-black text-white uppercase tracking-tight italic">{user.hero_class}</h3>
             <div className="flex items-center gap-2 mt-1">
                <Trophy className="w-3.5 h-3.5 text-yellow-500" />
                <span className="text-xs font-black text-yellow-500 uppercase tracking-widest">{user.arena_tier} {user.arena_tier_level}</span>
             </div>
             <div className="flex gap-1 mt-2">
                {[1,2,3,4,5].map(s => (
                  <div key={s} className={cn("w-3 h-3 rounded-full border border-white/10", s <= user.arena_stars ? "bg-yellow-500 shadow-[0_0_8px_#eab308]" : "bg-white/5")} />
                ))}
             </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3 mt-6">
           <div className="bg-white/5 p-3 rounded-xl border border-white/5 text-center">
              <p className="text-[8px] uppercase font-black text-neutral-500 mb-1">ATK</p>
              <p className="text-sm font-black text-white">{user.hero_attack}</p>
           </div>
           <div className="bg-white/5 p-3 rounded-xl border border-white/5 text-center">
              <p className="text-[8px] uppercase font-black text-neutral-500 mb-1">DEF</p>
              <p className="text-sm font-black text-white">{user.hero_defense}</p>
           </div>
           <div className="bg-white/5 p-3 rounded-xl border border-white/5 text-center">
              <p className="text-[8px] uppercase font-black text-neutral-500 mb-1">HP</p>
              <p className="text-sm font-black text-white">{user.hero_health}</p>
           </div>
        </div>

        <button 
          onClick={handleUpgrade}
          className="w-full mt-6 py-4 bg-white/5 border border-white/10 hover:bg-white/10 rounded-2xl flex items-center justify-between px-6 transition-all active:scale-[0.98]"
        >
          <div className="text-left">
            <p className="text-[9px] uppercase font-black text-yellow-500 tracking-tighter">Enhance Character</p>
            <p className="text-xs font-black text-white">UPGRADE TO LVL {user.hero_level + 1}</p>
          </div>
          <div className="text-right">
             <p className="text-xs font-mono font-black text-white">{nextUpgradeCost.toLocaleString()} <span className="text-[8px] text-yellow-500">GLDp</span></p>
          </div>
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="glass-card p-4 bg-white/5 border-white/5">
           <div className="flex items-center gap-2 mb-3">
              ...
              <p className="text-[10px] font-black uppercase text-neutral-400">Match Energy</p>
           </div>
           <div className="flex items-center justify-between">
              <p className="text-2xl font-black text-white">{freeMatchesLeft}</p>
              <span className="text-[8px] uppercase font-black px-2 py-1 bg-white/10 rounded-full text-neutral-400">FREE / 10</span>
           </div>
        </div>
        <div className="glass-card p-4 bg-white/5 border-white/5 relative overflow-hidden group">
           <div className="flex items-center gap-2 mb-3">
              <Target className="w-4 h-4 text-emerald-500" />
              <p className="text-[10px] font-black uppercase text-neutral-400">Elite Quota</p>
           </div>
           <div className="flex items-center justify-between">
              <p className="text-2xl font-black text-white">{5 - (user.combat_matches_ads || 0)}</p>
              <span className="text-[10px] uppercase font-black text-emerald-500 animate-pulse">RESERVE</span>
           </div>
        </div>
      </div>
